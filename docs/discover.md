...
## Filterable exploration

Collection cards now open the same Explore grid with editable presets. Explore all opens without a collection constraint. Mood, genre, decade and the curated awards selection can be combined; the movie/TV toggle is retained when other filters change. Clear filters resets the draft until Apply. Browse keeps known titles, using favorites and ratings only to order results within each page.

`GET /catalog/discover/browse` validates mood, genre, decade (1980 through 2020), awards and page (1 through 50). It uses optional verified identity and private/no-store headers. Browse moods are broad genre-based starting points, distinct from the six-title editorial pools on the Discover home. A restrictive combination may have no results; it is never broadened silently. Awards remain a small curated selection.

On Discover, the app prepares the default Explore page, the four collection presets and the active mood using the same user-scoped cache keys as the destination. Each selection prefetches at most eight posters. The first twelve visible recommendations prepare poster/backdrop images and detail data in batches of three. Concurrent openings reuse in-flight requests. Arbitrary combinations and later pagination are fetched on demand; cold network requests cannot be guaranteed instantaneous.
