...
export function selectRecentProgress(items: ProgressItem[]) {
  return items.filter(canResumeProgress)
    .sort((a, b) => (b.media.lastWatchedAt ?? '').localeCompare(a.media.lastWatchedAt ?? ''))
    .slice(0, 6);
}

export function canResumeProgress(item: ProgressItem) {
  return Boolean((item.next || (item.error && item.state === 'progress')) && item.media.watchedEpisodeCount > 0);
}
