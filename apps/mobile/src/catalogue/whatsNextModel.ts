...
import type { MovieViewingSummary, SeriesViewingSummary } from '../api/viewings';

export function getMovieCycleWatchedIds(
  watchedIds: ReadonlySet<number>,
  current: MovieViewingSummary,
  viewings: MovieViewingSummary[],
) {
  const cycleStart = current.viewCount > 1 ? current.latestLoggedAt : null;
  if (!cycleStart) return watchedIds;
  return new Set(viewings.filter((item) => watchedIds.has(item.tmdbId)
    && item.latestLoggedAt && item.latestLoggedAt >= cycleStart).map((item) => item.tmdbId));
}

export function getSeriesRewatchAnchor(episodes: NonNullable<SeriesViewingSummary['episodes']>) {
  return episodes.filter((item) => item.seasonNumber > 0 && item.viewCount > 1)
    .reduce<(typeof episodes)[number] | null>((latest, item) =>
      !latest || item.latestLoggedAt > latest.latestLoggedAt ? item : latest, null);
}
...
export async function findNextSeriesEpisode(
  ...
  today = new Date().toISOString().slice(0, 10),
  rewatchAnchor?: { seasonNumber: number; episodeNumber: number } | null,
) {
  ...
  const isWatched = (seasonNumber: number, episodeNumber: number) =>
    Boolean(rewatchAnchor && (seasonNumber < rewatchAnchor.seasonNumber
      || seasonNumber === rewatchAnchor.seasonNumber && episodeNumber <= rewatchAnchor.episodeNumber))
    || watchedKeys.has(`${seasonNumber}:${episodeNumber}`);
  ...
  for (const season of ordered) {
    ...
    if (count > 0 && Array.from({ length: count }, (_, index) => index + 1)
      .every((episode) => isWatched(season.seasonNumber, episode))) continue;
    ...
    const next = [...details.episodes]
      ...
      .find((episode) => episode.airDate && episode.airDate <= today
        && !isWatched(season.seasonNumber, episode.episodeNumber));
