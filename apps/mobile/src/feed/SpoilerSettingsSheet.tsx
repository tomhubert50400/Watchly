    ...
    <BottomActionSheet visible={visible} onClose={onClose} title="Spoiler protection">
      <BottomActionSheetScrollView>
        ...
        <View style={styles.row}>
          ...
          <Switch accessibilityLabel="Enable spoiler protection" disabled={!state.loaded || state.saving} value={preferences.enabled} onValueChange={(enabled) => void state.save({ ...preferences, enabled })} trackColor={{ true: colors.accent }} thumbColor={colors.text} />
        </View>
        {preferences.enabled ? <>
          {choices.map(([key, label]) => <View key={key} style={styles.row}>
            <Text style={styles.label}>{label}</Text>
            <Switch accessibilityLabel={label} disabled={state.saving} value={preferences[key]} onValueChange={(value) => void state.save({ ...preferences, [key]: value })} trackColor={{ true: colors.accent }} thumbColor={colors.text} />
          </View>)}
          <Text style={styles.label}>Recent releases</Text>
          <View style={styles.options}>
            {([0, 3, 7, 14] as const).map((days) => <Button compact key={days} label={days ? `${days} days` : 'Off'} accessibilityState={{ selected: preferences.recentDays === days }} disabled={state.saving} variant={preferences.recentDays === days ? 'primary' : 'secondary'} onPress={() => void state.save({ ...preferences, recentDays: days })} />)}
          </View>
