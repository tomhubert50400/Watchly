...
assert(resolveCarriedPosterTilt(500) > 0, 'rightward movement must apply the visually opposing swing');
assert(resolveCarriedPosterTilt(-500) < 0, 'leftward movement must apply the visually opposing swing');
assert.equal(resolveCarriedPosterTilt(10_000), 24, 'carried poster rotation must stay controlled');
...
assert.match(screen, /Animated\.spring\(dragScale,[\s\S]*?useNativeDriver: false/, 'lift and swing must use the same animation driver');
