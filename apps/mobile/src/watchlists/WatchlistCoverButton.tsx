...
function WatchlistArtworkSheet({
  ...
  const selected = mode === 'cover' ? coverSelection : backgroundSelection;
  ...
  useEffect(() => {
    ...
    const candidates = [...new Map([...visible, ...getWatchlistCoverItems(items, selected)].map((item) => [item.id, item])).values()];
    async function load() {
      setLoading(true);
      setArtwork({});
      for (let index = 0; index < candidates.length; index += 6) {
        ...
        const results = await Promise.all(candidates.slice(index, index + 6).map(async (item) => {
          try {
            ...
            const artworkUrl = mode === 'cover'
              ? media.backdropUrl ?? media.posterUrl
              : media.posterUrl ?? media.backdropUrl;
            return [item.id, { artworkUrl, title: media.title }] as const;
          } catch {
  ...
  }, [items, mode, visibleCount, refreshMovie, refreshSeries, retry]);

  function updateSelection(updater: (current: string[]) => string[]) {
    if (mode === 'cover') setCoverSelection(updater);
    else setBackgroundSelection(updater);
  }
  ...
  return <BottomActionSheet visible title="Customize watchlist" onClose={onClose}
    ...
    <BottomActionSheetScrollView contentContainerStyle={styles.content}>
      <SegmentedControl
        options={[
          { label: 'Change cover', value: 'cover' },
      ...
      />
      <View
        accessibilityLabel={`${mode === 'cover' ? 'Cover' : 'Background'} preview`}
        style={[styles.preview, mode === 'background' ? styles.backgroundPreview : null]}
      >
        {mode === 'cover' ? (
      ...
      <View style={styles.grid}>
        {items.slice(0, visibleCount).map((item) => {
          ...
          return <Pressable key={item.id} accessibilityRole="checkbox"
            ...
            disabled={disabled} onPress={() => updateSelection((previous) => mode === 'cover'
              ? toggleWatchlistCoverItem(previous, item.id)
              : previous[0] === item.id ? [] : [item.id])}
            style={[styles.tile, mode === 'background' ? styles.backgroundTile : null, disabled && position < 0 && styles.dimmed]}>
            <View style={[styles.posterFrame, position >= 0 && styles.selected]}>
              <MediaPoster
                posterUrl={media?.artworkUrl ?? null}
                style={mode === 'cover' ? styles.coverArtwork : styles.backgroundArtwork}
              />
...
const styles = StyleSheet.create({
  backgroundArtwork: { width: '100%', aspectRatio: 2 / 3 },
  backgroundPreview: { alignSelf: 'center', aspectRatio: 2 / 3, width: '46%' },
